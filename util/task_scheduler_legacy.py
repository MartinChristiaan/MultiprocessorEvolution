...
nodes = [ARMv8(2/3),Adreno(1),Adreno(1),MIPS(1),ARMv8(2/3),MIPS(1)]
...
print(nodes[3].time_per_task_s1)
...
def schedule_task(schedule,t_completions_t1,t_completions_t2):
    ...
    for i_node,node in enumerate(nodes):
        ...
        for i_task,node_assigned in enumerate(schedule):
            if node_assigned == i_node:
                time_task_available_t1 = max(t_completions_t1[i_task],time_task_available_t1)
    ...
    if task_id == len(dependencies)-1:
        ...
        t_completions_t1+=potential_times_t1[best_node]
        t_completions_t2+=potential_times_t2[best_node]
        T_final =  t_completions_t1 + t_completions_t2

        return schedule,max(T_final)
    else:
