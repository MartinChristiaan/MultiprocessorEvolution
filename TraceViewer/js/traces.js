...
function addPowerTrace(power, xml) {
    var currentAmount = 0;
    var data =[];
    power.push({lines: { show: true, fill: true }, label: 'Battery Power Drain', name: 'Battery Power Drain', data : data});
    $(xml).find("trace").children().each(
            function () {
                switch (this.nodeName) {
                    case "power":
                        ...
                        currentAmount += parseFloat(this.getAttribute("difference"));
                        data.push([t, currentAmount]);
                        break;
                }
            }
...
function addMemTrace(mem, xml, id, color) {
    var currentAmount = 0;
    var data =[];
    data.push([0, currentAmount]); // Always start at 0 memory at time 0
